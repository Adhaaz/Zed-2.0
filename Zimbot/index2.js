...
async function ManggaToon(judul) {
	try {
		...
		if(id === 'undefined') {
			...
			let Author = author.replace('Name Author: ', '');
			let hasil = {
...
async function RamalJadian(tanggal, bulan, tahun) {
	...
	let a = tanggall.replace('MEANING OF BIRTHDAY, WEDDING', '').replace('Characteristics:', '\nCharacteristics : ').replace('< Count Back', '')
	return a
}
...
function trutdare(querry) {
	...
	return res
		...
		return `Querry not listed in the menu please enter correctly.\n\nhow to use :\n1. trutdare('truth id')\n2. trutdare('truth eng')\n3. trutdare('dare id')\n4. trutdare('dare eng')`
	}
}
...
async function Otakudesu(querry) {
	...
	var notFond = {
		author: 'Zed Bot',
		status: link.status,
		Pesan: 'ERROR CONTACT OWNER +263718635356'
	}
	return notFond
}
}
